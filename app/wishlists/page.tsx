...
import type { Product, WishList } from '@/lib/features/types'
    ...
    return (
        <Navigation>
            <div className='w-full flex flex-col justify-center items-center'>
                <div className='w-full bg-[rgba(232,41,138,0.25)] h-[2rem] text-center flex justify-center items-center'>
                    <p className='text-base'>Our Big Little Event is now on | Shop up to 40% off</p>
                </div>

                <div className='mt-[2rem] w-full max-w-[90rem] px-[1rem] lg:px-[3rem]'>
                    ...
                    <div className='mt-[2rem]'>
                        <p className='text-base md:text-[1.125rem]'>It's super simple to create a wishlist! When you're browsing and see something you love, click the heart icon to add that item to your list. You can even login and drop a little hint by sharing your wish list with your nearest and dearest!</p>

                        <div className='mt-[2rem] py-[2rem]'>
                            <div>
                                {wishLists?.map((wishlist: WishList, index: number) => {
                                    return <div key={index.toString()} className="">
                                        ...
                                        <div className='mt-[1.5rem] grid grid-cols-2 gap-x-[0.5rem] gap-y-9 md:gap-x-6 lg:gap-x-[2rem] sm:grid-cols-2 md:grid-cols-3'>
                                            {
                                                wishlist?.products.map((product: Product, index: number) => {
                                                    return <Link key={index?.toString()} href={`/product/${product?.product_id}`}>
                                                        <div className="overflow-hidden flex flex-col justify-center items-center">
                                                            {
                                                                product && product?.urls !== undefined && product?.urls.length > 0 && <div className="relative w-full h-auto min-h-[15.5rem] sm:min-h-[21rem] md:min-h-[21.875rem] lg:min-h-[25.875rem]">
                                                                    <Image
                                                            ...
                                                            <div className=" pb-3 flex flex-col gap-y-[0.5rem] mt-[0.75rem] w-full justify-center">
                                                                <h3 className="text-base font-semibold">{product.name}</h3>
                                                                <p className="text-[1.125rem] font-normal">{product.description}</p>
                                                                <p className="text-[1.25rem] font-bold">KES {product.price}</p>

                                                                <div className='flex justify-between items-center'>
                                                                    <Button className="w-[5.3rem] md:w-[11rem] md:items-center rounded-[0.125rem] bg-[#AF52DE] h-[2rem] font-semibold flex gap-x-[0.25] md:gap-[0.75rem]">
                                                                    ...
                                                                    <Button className='bg-white border border-[#7A4597] h-[2rem] md:w-[5rem] text-[#7A4597] text-base'>
                                                                        Remove
                                                                    </Button>
