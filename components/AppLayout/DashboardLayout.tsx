...
const dashboardTabs = [
  ...
  { name: "Wishlist", href: "/dashboard/wishlists", icon: <Heart className="h-4 w-4" /> },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col w-full min-h-screen justify-start items-center">
      <div className="w-full max-w-[90rem] px-4 py-3 border-b flex items-center justify-between">
        <Breadcrumb>
          <BreadcrumbList>
            <BreadcrumbItem>
                ...
                return (
                  <Link key={tab.name} href={tab.href}>
                    <Button
                      ...
                      className={`w-full justify-start rounded-sm h-[2.5rem] gap-2 ${isActive ? "bg-[#AF52DE] text-custom-black hover:bg-[#B085F5]" : "hover:bg-muted"}`}
