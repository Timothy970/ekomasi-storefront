...
export default function SideBar({ isOpen, setIsOpen }: AppHeaderProps) {
    return (
        <div className={`lg:hidden fixed px-[1.25rem] pb-[2.5rem]  top-0 left-0 h-full w-full bg-white overflow-hidden text-black transform transition-transform duration-500 ease-in-out z-60 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
            <div className="flex justify-between items-start mt-[1rem] w-full]">
                <div className='flex justify-start items-center mb-[1.25rem]'>
                    ...
                    <h2 className='text-black text-center font-poppins text-sm font-normal leading-[150%]'>Hi User!</h2>
                </div>
