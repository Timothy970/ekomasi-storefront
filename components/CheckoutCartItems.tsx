...
export default function ChekcoutCartItems({ cart }: Readonly<CartProps>) {
            ...
            <div className='w-full flex flex-col gap-y-[0.5rem]'>
                {
                    cart?.cart_items?.map((item) => {
                        return <CartItemProducts hideBtns={true} key={item.product_id || item.product?.product_id || item.sku} item={item} />
                    })
                }
            </div>
        </div>
