      ...
      <div className="max-w-[90rem] w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex flex-col gap-y-[0.5rem] sm:gap-y-[1rem]">
            <h2 className="text-[1.5rem] sm:text-[1.875rem] lg:text-[2.25rem] not-italic font-bold leading-tight gold-shimmer">Categories</h2>
            <p className="text-muted-foreground font-poppins text-[0.875rem] sm:text-[1rem] lg:text-[1.125rem] font-normal leading-tight">
              Explore our signature perfume collections, niche oils, and luxury scents
            </p>
          </div>

          <Button onClick={() => router.push("/categories")} className="h-[2.5rem] sm:h-[2.75rem] lg:h-[3rem] hidden sm:flex min-w-[10rem] lg:min-w-[12rem] bg-transparent text-foreground hover:bg-muted justify-center items-center border rounded-full border-border">
            <span className="text-[0.875rem] sm:text-[1rem] font-medium">View All</span>
          </Button>
        </div>
              ...
              <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-[0.5rem] sm:gap-[0.75rem] md:gap-[1rem]">
                {firstRow?.map((category) => (
                  <Link key={category.id} href={`/category/${category?.id}`}>
                    <div
                      className="category-card overflow-hidden bg-white rounded-lg border border-border/60 shadow-xs hover:shadow-md transition-all duration-300 hover:scale-[1.02]"
                    >
