    ...
    <div className="w-full flex justify-center items-center mt-[2.5rem]">
      <div className="max-w-[90rem] w-full px-[1rem] lg:px-[3rem]">
        ...
        <div className="mt-[2rem]">
          <div className="grid grid-cols-2 gap-x-[0.5rem] gap-y-9 md:gap-x-6 lg:gap-x-[2rem] sm:grid-cols-2 md:grid-cols-3">
            {categories?.map((category) => (
              <Link key={category.id} href={`/category/${category?.id}`}>
                <div className="overflow-hidden flex flex-col justify-center items-center bg-white border border-gray-200 lg:border-0 hover:shadow-xl transform transition duration-300 ease-in-out">
                  <div className="relative w-full h-auto min-h-[15.5rem] sm:min-h-[21rem] md:min-h-[21.875rem] rounded-none lg:rounded-tl-lg lg:rounded-tr-lg">
                    <Image
