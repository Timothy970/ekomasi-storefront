...
export default function VouchersTable({ data, pagination }: Readonly<VouchersTableProps>) {
                ...
                <table className="w-full">
                    ...
                    <tbody className='w-full'>
                        {table.getRowModel().rows.map((row) => (
                            <tr key={row.id} className='h-[3rem]'>
                                {row.getVisibleCells().map((cell) => (
                                    <td key={cell.id} className="border border-l-0 border-r-0 p-2 text-start">
